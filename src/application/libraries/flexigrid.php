...
class Flexigrid
{
	...
	*/	
	...
	public function validate_post() 
	{
		...
		$d_sortname=$this->CI->default_sortname;
		$d_sortorder=$this->CI->default_sortorder;
		...
		{
			if (is_array($this->CI->default_valid_fields))
			{
				if (isset($this->CI->default_valid_fields[$this->CI->input->post('sortname')]['sorteable']) && $this->CI->default_valid_fields[$this->CI->input->post('sortname')]['sorteable'] === TRUE) 
				{
	...
	{
		if($sortname == FALSE && $this->CI->session->userdata('sortname_'.$this->CI->router->class)!=FALSE  ){
			$sortname = $this->CI->session->userdata('sortname_'.$this->CI->router->class);
			$_POST['sortname'] = $sortname;
				
		}
		
		if($sortorder == FALSE && $this->CI->session->userdata('sortorder_'.$this->CI->router->class)!=FALSE ){
			$sortorder = $this->CI->session->userdata('sortorder_'.$this->CI->router->class);
	...
	*/
	/**
	 * Query builder. Adds sort, search 
	 *
	 * @access	public
	 * @param	objeto doctrine get_all()
	 * @return	nothing
	 */
	...
	public function build_query()
	{
		...
		if(is_array($this->CI->session->userdata('filtro_'.$this->CI->router->class))){
			$filtro=$this->CI->session->userdata('filtro_'.$this->CI->router->class);
			while(list(,$where) =each($filtro)){
				if(is_array($where[1]))
				{
					//$this->CI->query->addWhere($where[0],$where[1]);
					$this->CI->query->whereIn($where[0],$where[1]);
				}else{
					$this->CI->query->addWhere($where[0],$where[1]);	
				}
		...
		$this->CI->query->orderby($this->post_info['sortname'].' '.$this->post_info['sortorder'] );
		//guardo ultimo orden para usarlo por default
		$this->CI->session->set_userdata('sortname_'.$this->CI->router->class,$this->post_info['sortname']);
		$this->CI->session->set_userdata('sortorder_'.$this->CI->router->class,$this->post_info['sortorder']);
		
		#limit para el paginador de doctrine
		$this->CI->post_info['rp']=$this->post_info['rp'];
		$this->CI->post_info['page']=$this->post_info['page'];
	} 
	...
	public function build_query($limit = TRUE)
	{
		...
		if ($this->post_info['swhere']){
			...
			$return['where']=$this->post_info['swhere'];
			$this->post_info['swhere'];
		...
		$return['sortname']=$this->post_info['sortname'];
		$return['sortorder']=$this->post_info['sortorder'];
		...
		$return['rp']=$this->post_info['rp'];
		$return['page']=$this->post_info['page'];
	...
	*/
	...
	public function init_json_build($record_count)
	{
		...
		{
			$this->json_build = "{";
			$this->json_build .= __LINE__.__FILE__."page: ".$this->post_info['page'].",";
