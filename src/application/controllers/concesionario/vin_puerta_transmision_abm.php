<?php
/*
todo id unico - DONE, tambien chequea la cantidad de digitos
*/

define('ID_SECCION',1065);
class Vin_Puerta_Transmision_Abm extends Backend_Controller{
	...
	public function add()
	{
		...
		{		
			if ($this->_validar_formulario() == TRUE)
			{
				...
				$this->registro_actual = new Vin_Puerta_Transmision();
	...
	private function _validar_formulario() {
		...
		$this->form_validation->set_rules('id',$this->marvin->mysql_field_to_human('id'),
				'trim|exact_length[1]|required|my_unique_db[Vin_Puerta_Transmision.id '.$id.']' );
