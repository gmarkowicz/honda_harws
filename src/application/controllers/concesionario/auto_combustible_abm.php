<?php
define('ID_SECCION',1041);
class Auto_Combustible_abm extends Backend_Controller{
	...
	public function add()
	{
		...
		{		
			if ($this->_validar_formulario() == TRUE)
			{
				...
				$this->registro_actual = new Auto_Combustible();
	...
	private function _grabar_registro_actual()
	{
		try
		{
			...
			*/
			$this->registro_actual->auto_combustible_field_desc = $this->input->post('auto_combustible_field_desc');
	...
	private function _validar_formulario() {
		...
		$this->form_validation->set_rules('auto_combustible_field_desc',$this->marvin->mysql_field_to_human('auto_combustible_field_desc'),
				'trim|max_length[255]|required|my_unique_db[Auto_Combustible.auto_combustible_field_desc '.$id.']' );
