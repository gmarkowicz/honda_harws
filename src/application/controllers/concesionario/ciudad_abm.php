<?php
define('ID_SECCION',1031);
class Ciudad_abm extends Backend_Controller{
	...
	public function add()
	{
		...
		{		
			if ($this->_validar_formulario() == TRUE)
			{
				...
				$this->registro_actual = new Ciudad();
	...
	private function _grabar_registro_actual()
	{
		try
		{
			...
			*/
			$this->registro_actual->ciudad_field_desc = $this->input->post('ciudad_field_desc');
	...
	private function _validar_formulario() {
		...
		$this->form_validation->set_rules('ciudad_field_desc',$this->marvin->mysql_field_to_human('ciudad_field_desc'),
				'trim|max_length[255]|required|callback_valid_ciudad' );

		$this->form_validation->set_rules('provincia_id',$this->marvin->mysql_field_to_human('provincia_id'),
				'trim|required|my_db_value_exist[Provincia.id]' );
	...
	function valid_ciudad($ciudad)
	{
		
		$this->form_validation->set_message('valid_ciudad', $this->lang->line('form_campo_duplicado'));
		...
		->from('Ciudad')
		->where('ciudad_field_desc = ?',$ciudad)
		->addWhere('provincia_id = ?',$this->input->post('provincia_id'))
		->addWhere('id != ?',$id);
