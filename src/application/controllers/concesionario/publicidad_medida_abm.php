<?php
define('ID_SECCION',4014);
class Publicidad_Medida_Abm extends Backend_Controller{
	...
	public function add()
	{
		...
		{		
			if ($this->_validar_formulario() == TRUE)
			{
				...
				$this->registro_actual = new Publicidad_Medida();
	...
	private function _grabar_registro_actual()
	{
		try
		{
			...
			{
				$this->registro_actual->publicidad_medida_field_admin_alta_id = $this->session->userdata('admin_id');
				$this->registro_actual->publicidad_medida_field_fechahora_alta = date('Y-m-d H:i:s', time());
			}
			else if($this->router->method == 'edit' )
			{
				$this->registro_actual->publicidad_medida_field_admin_modifica_id = $this->session->userdata('admin_id');
				$this->registro_actual->publicidad_medida_field_fechahora_modificacion = date('Y-m-d H:i:s', time());
			}
			*/
			$this->registro_actual->publicidad_medida_field_desc = $this->input->post('publicidad_medida_field_desc');
	...
	private function _validar_formulario() {
		...
		$this->form_validation->set_rules('publicidad_medida_field_desc',$this->marvin->mysql_field_to_human('publicidad_medida_field_desc'),
				'trim|max_length[255]' );
