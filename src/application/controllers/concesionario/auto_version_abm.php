<?php
define('ID_SECCION',1048);
class Auto_Version_Abm extends Backend_Controller{
	...
	public function add()
	{
		...
		{		
			if ($this->_validar_formulario() == TRUE)
			{
				...
				$this->registro_actual = new Auto_Version();
	...
	private function _grabar_registro_actual()
	{
		try
		{
			...
			*/
			$this->registro_actual->auto_version_field_desc = $this->input->post('auto_version_field_desc');
	...
	private function _view()
	{
		...
		$q->orderBy('auto_marca_field_desc ASC');
		$q->addOrderBy('auto_modelo_field_desc ASC');
	...
	private function _validar_formulario() {
		...
		$this->form_validation->set_rules('auto_version_field_desc',$this->marvin->mysql_field_to_human('auto_version_field_desc'),
