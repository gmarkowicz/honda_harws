...
*/
define('ID_SECCION',1062);
class Vin_Fabrica_abm extends Backend_Controller{
	...
	public function add()
	{
		...
		{		
			if ($this->_validar_formulario() == TRUE)
			{
				...
				$this->registro_actual = new Vin_Fabrica();
	...
	private function _validar_formulario() {
		...
		$this->form_validation->set_rules('id',$this->marvin->mysql_field_to_human('id'),
				'trim|exact_length[1]|required|my_unique_db[Vin_Fabrica.id '.$id.']' );
