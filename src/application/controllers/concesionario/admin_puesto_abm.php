<?php
define('ID_SECCION',1015);
class Admin_Puesto_abm extends Backend_Controller{
	...
	public function add()
	{
		...
		{		
			if ($this->_validar_formulario() == TRUE)
			{
				...
				$this->registro_actual = new Admin_Puesto();
	...
	private function _grabar_registro_actual()
	{
		try
		{
			...
			*/
			$this->registro_actual->admin_puesto_field_desc = $this->input->post('admin_puesto_field_desc');
			...
			if($this->registro_actual!=FALSE && is_array($this->input->post('many_admin')))
			{
					foreach($this->input->post('many_admin') as $admin_id) {
						...
						$relacion->admin_puesto_id = $this->registro_actual->id;
						$relacion->save();
					}
			}
	...
	private function _validar_formulario() {
		...
		$this->form_validation->set_rules('admin_puesto_field_desc',$this->marvin->mysql_field_to_human('admin_puesto_field_desc'),
