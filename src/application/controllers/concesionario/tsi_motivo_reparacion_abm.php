<?php
define('ID_SECCION',3022);
class Tsi_Motivo_Reparacion_Abm extends Backend_Controller{
	...
	public function add()
	{
		...
		{		
			if ($this->_validar_formulario() == TRUE)
			{
				...
				$this->registro_actual = new Tsi_Motivo_Reparacion();
	...
	private function _grabar_registro_actual()
	{
		try
		{
			...
			*/
			$this->registro_actual->tsi_motivo_reparacion_field_desc = $this->input->post('tsi_motivo_reparacion_field_desc');
	...
	private function _validar_formulario() {
		...
		$this->form_validation->set_rules('tsi_motivo_reparacion_field_desc',$this->marvin->mysql_field_to_human('tsi_motivo_reparacion_field_desc'),
				'trim|max_length[255]|required|my_unique_db[Tsi_Motivo_Reparacion.tsi_motivo_reparacion_field_desc '.$id.']' );
