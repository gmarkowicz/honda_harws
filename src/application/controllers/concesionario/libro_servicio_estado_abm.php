<?php
define('ID_SECCION',3042);
class Libro_Servicio_Estado_Abm extends Backend_Controller{
	...
	public function add()
	{
		...
		{		
			if ($this->_validar_formulario() == TRUE)
			{
				...
				$this->registro_actual = new Libro_Servicio_Estado();
	...
	private function _grabar_registro_actual()
	{
		try
		{
			...
			*/
			$this->registro_actual->libro_servicio_estado_field_desc = $this->input->post('libro_servicio_estado_field_desc');
	...
	private function _validar_formulario() {
		...
		$this->form_validation->set_rules('libro_servicio_estado_field_desc',$this->marvin->mysql_field_to_human('libro_servicio_estado_field_desc'),
				'trim|max_length[255]|required|min_length[4]' );
