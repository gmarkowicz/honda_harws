...
class Reclamo_Garantia_Campania_Main extends Backend_Controller {
	...
	//error mensaje
	var $error_mensaje = false;

	var $info_csv = false;

	//columnas por las que se puede ordenar la data //List of all fields that can be sortable
	var $default_valid_fields = array(
			'_buscador_general' => array(
					'sql_filter' => array(
							'MATCH(reclamo_garantia_campania_field_desc) AGAINST (? IN BOOLEAN MODE)'),
					//definir campos a buscar
					'grid' => FALSE, 'width' => FALSE,
					'sorteable' => FALSE, 'align' => FALSE,
					'export' => FALSE, 'download' => FALSE,
					'print' => FALSE, 'rules' => FALSE),
			'reclamo_garantia_campania_field_desc' => array(
					'sql_filter' => array(
							'MATCH(reclamo_garantia_campania_field_desc) AGAINST (? IN BOOLEAN MODE)'),
					//definir campos a buscar
					'grid' => TRUE, 'width' => 750, 'sorteable' => TRUE,
					'align' => 'left', 'export' => TRUE, 'download' => FALSE,
					'print' => TRUE,
	...
	function __construct() {
		...
		$this->template['js_grid'] = build_grid_js('flex1',
				//html name
				$this->get_grid_url(), //url a la que apunta
				$esqueleto_grid, $this->default_sortname,
				//default order
	...
	function Reclamo_Garantia_Campania_Main() {
	...
	private function _validar_filtros() {
		...
		$this->form_validation
				->set_rules('_buscador_general',
						$this->marvin
								->mysql_field_to_human('_buscador_general'),
						'trim');
	...
	function index() {
		...
		if ($this->input->post('_filtro')) {
			if ($this->_validar_filtros()) { //validamos los datos que envia...
				...
				if (count($filtro) > 0) {
					...
					$this->session
							->set_userdata('filtro_' . $this->router->class,
									$filtro);
				}
				#--------------------------
			}
	...
	function grid() {
		...
		//-------------------------[escribe la grilla]
		$pager = new Doctrine_Pager($this->query, $this->post_info['page'],
				//flexigrid genera este dato
		...
		foreach ($resultado as $row) {
			$record_items[] = $this
					->_create_grid_data_zip($row, $config['campos']);
		}
		...
		$this->output
				->set_output(
						$this->flexigrid
								->json_build($pager->getNumResults(),
										$record_items));
		//-------------------------[/escribe la grilla]
	}
	...
	function export() {
		...
		while (list(, $field_name) = each($export_fields)) {
			$this->ofimatica
					->add_header(
							$this->marvin->mysql_field_to_human($field_name),
							$config['campos'][$field_name]['width']);
		}
		//creo el header
		...
		if ($maximos_registos > 0 && $total > $maximos_registos)
			$total = $maximos_registos;
