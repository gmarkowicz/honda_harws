...
class Tsi_Tipo_Mantenimiento_Abm extends Backend_Controller{
	...
	public function add()
	{
		...
		{		
			if ($this->_validar_formulario() == TRUE)
			{
				...
				$this->registro_actual = new Tsi_Tipo_Mantenimiento();
	...
	private function _grabar_registro_actual()
	{
		try
		{
			...
			*/
			$this->registro_actual->tsi_tipo_mantenimiento_field_desc = $this->input->post('tsi_tipo_mantenimiento_field_desc');
	...
	private function _validar_formulario() {
		...
		$this->form_validation->set_rules('tsi_tipo_mantenimiento_field_desc',$this->marvin->mysql_field_to_human('tsi_tipo_mantenimiento_field_desc'),
				'trim|max_length[255]|required|my_unique_db[Tsi_Tipo_Mantenimiento.tsi_tipo_mantenimiento_field_desc '.$id.']' );
