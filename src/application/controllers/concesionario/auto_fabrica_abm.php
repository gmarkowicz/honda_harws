<?php
define('ID_SECCION',1044);
class Auto_Fabrica_abm extends Backend_Controller{
	...
	public function add()
	{
		...
		{		
			if ($this->_validar_formulario() == TRUE)
			{
				...
				$this->registro_actual = new Auto_Fabrica();
	...
	private function _grabar_registro_actual()
	{
		try
		{
			...
			*/
			$this->registro_actual->auto_fabrica_field_desc = $this->input->post('auto_fabrica_field_desc');
	...
	private function _validar_formulario() {
		...
		$this->form_validation->set_rules('auto_fabrica_field_desc',$this->marvin->mysql_field_to_human('auto_fabrica_field_desc'),
