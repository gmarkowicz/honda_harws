...
class Vin_Procedencia_Ktype_Main extends Backend_Controller {		
	//filtra por sucursal?
	var $sucursal = FALSE;
		
	//modelo doctrine con el cual laburamos
	var $model = 'Vin_Procedencia_Ktype';
			...
			array(
				'sql_filter'	=>array(
										...
										'MATCH( 
												...
												vin_procedencia_ktype_field_ktype
	...
	function Vin_Procedencia_Ktype_Main()
	{
	...
	private function _validar_filtros() {		
		...
		$campos = $this->default_valid_fields;
		reset($campos);
		while(list($k,)=each($campos))
		{
			$this->form_validation->set_rules($k,$this->marvin->mysql_field_to_human($k),
			'trim'
			);
		}
