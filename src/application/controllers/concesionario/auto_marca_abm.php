<?php
define('ID_SECCION',1045);
class Auto_Marca_abm extends Backend_Controller{
	...
	public function add()
	{
		...
		{		
			if ($this->_validar_formulario() == TRUE)
			{
				...
				$this->registro_actual = new Auto_Marca();
	...
	//----------------------------------------------------------------
	//-------------------------[edita el registro]
	public function oldedit($id = FALSE)
	{
		$this->_set_record($id);
		if($this->input->post('_submit'))
		{
	...
	private function _grabar_registro_actual()
	{
		try
		{
			...
			*/
			$this->registro_actual->auto_marca_field_desc = $this->input->post('auto_marca_field_desc');
	...
	private function _validar_formulario() {
		...
		$this->form_validation->set_rules('auto_marca_field_desc',$this->marvin->mysql_field_to_human('auto_marca_field_desc'),
				'trim|max_length[255]|required|my_unique_db[Auto_Marca.auto_marca_field_desc '.$id.']' );
