...
*/
define('ID_SECCION',6014);
class Boutique_Categoria_Abm extends Backend_Controller{
	...
	public function add()
	{
		if($this->input->post('_submit'))
		{
			...
			if ($this->_validar_formulario() == TRUE)
			{
				...
				$this->registro_actual = new Boutique_Categoria();
	...
	private function _grabar_registro_actual()
	{
		try
		{
			...
			//Valores obtenidos del post
			$this->registro_actual->boutique_categoria_field_desc = $this->input->post('boutique_categoria_field_desc');
			$this->registro_actual->boutique_categoria_estado_id  = $this->input->post('boutique_categoria_estado_id');

			//Guarda los datos del post
	...
	private function _view()
	{
		...
		$boutique_categoria_estado = new Boutique_Categoria_Estado();
		$q = $boutique_categoria_estado->get_all();
		//$q->WhereIn(' sucursal_id ', $this->session->userdata('sucursales'));
		$config=array();
		$config['fields'] = array('boutique_categoria_estado_field_desc');
		$config['select'] = TRUE;
		$this->template['estado_id']=$this->_create_html_options($q, $config);
	...
	private function _validar_formulario() {
		...
		$this->form_validation->set_rules('boutique_categoria_field_desc',$this->marvin->mysql_field_to_human('boutique_categoria_field_desc'),
