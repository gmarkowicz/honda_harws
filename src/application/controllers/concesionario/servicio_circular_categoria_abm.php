<?php
define('ID_SECCION',3035);
class Servicio_Circular_Categoria_Abm extends Backend_Controller{
	...
	public function add()
	{
		...
		{		
			if ($this->_validar_formulario() == TRUE)
			{
				...
				$this->registro_actual = new Servicio_Circular_Categoria();
	...
	private function _grabar_registro_actual()
	{
		try
		{
			...
			*/
			$this->registro_actual->servicio_circular_categoria_field_desc = $this->input->post('servicio_circular_categoria_field_desc');
	...
	private function _validar_formulario() {
		...
		$this->form_validation->set_rules('servicio_circular_categoria_field_desc',$this->marvin->mysql_field_to_human('servicio_circular_categoria_field_desc'),
				'trim|max_length[255]' );
