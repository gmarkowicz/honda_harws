...
class Tsi_Encuesta_Seguimiento_Abm extends Backend_Controller{
	...
	{
		$this->_set_record($id);
		if(in_array($this->registro_actual->tsi_encuesta_seguimiento_estado_id,$this->bloqueados) && !$this->backend->_permiso('edit'))
		{
			if(isset($_POST))
				unset($_POST);
		}
	...
	function valid_tsi_fecha()
	{
		if($this->router->method == 'add' )
		{
			...
			{
				$this->form_validation->set_message('valid_tsi_fecha', 'Debe Seleccionar una Operación de Servicio');
				RETURN FALSE;
			}
			...
			->addWhere('TSI.tsi_estado_id != ?',9)
			//->addWhere('(TO_DAYS(NOW()) - TO_DAYS(TSI.tsi_field_fecha_de_egreso)) <= ?',15)
			->WhereIn('TSI.sucursal_id', $this->session->userdata('sucursales'));
			if($q->count()!=1)
			{
				$this->form_validation->set_message('valid_tsi_fecha', 'Operacion de Servicio Inexistente o Rechazada');
				RETURN FALSE;
			}
			else
			{
				$r=$q->fetchOne(array(),Doctrine_Core::HYDRATE_ARRAY);
				$egreso = new DateTime($r['tsi_field_fecha_de_egreso']);
				$hoy = new DateTime(date('Y-m-d'));
				$intervalo = date_diff($hoy, $egreso);
				if((int)$intervalo->format('%d')>15) //pasaron mas de 15 dias?
				{
					$this->form_validation->set_message('valid_tsi_fecha', 'La fecha de egreso del vehiculo es mayor a 15 días con respecto al día de hoy');
					RETURN FALSE;
				}
				// a ver si no existe una encuesta ya ingresada para este TSI.
				$id = FALSE;
				if($this->registro_actual)
					$id = $this->registro_actual->id;
				$q = Doctrine_Query::create()
				->from('Tsi_Encuesta_Seguimiento Tsi_Encuesta_Seguimiento')
				->where('Tsi_Encuesta_Seguimiento.tsi_id =  ?',$this->input->post('tsi_id'))
				->addWhere('Tsi_Encuesta_Seguimiento.id != ?',$id);
				if($q->count()!=0)
				{
					$this->form_validation->set_message('valid_tsi_fecha', 'Ya existe una encueta para la Operación de Servicio Seleccionada');
					RETURN FALSE;
				}						
			}
		}
