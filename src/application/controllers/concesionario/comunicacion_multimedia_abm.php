<?php
define('ID_SECCION',405);
class Comunicacion_Multimedia_Abm extends Backend_Controller{
	...
	public function add()
	{
		if($this->input->post('_submit'))
		{
			if ($this->_validar_formulario() == TRUE)
			{
				...
				$this->registro_actual = new Comunicacion_Multimedia();
	...
	private function _grabar_registro_actual()
	{
		try
		{
			...
			{
				$this->registro_actual->comunicacion_multimedia_field_admin_alta_id = $this->session->userdata('admin_id');
				$this->registro_actual->comunicacion_multimedia_field_fechahora_alta = date('Y-m-d H:i:s', time());
			}
			else if($this->router->method == 'edit' )
			{
				$this->registro_actual->comunicacion_multimedia_field_admin_modifica_id = $this->session->userdata('admin_id');
				$this->registro_actual->comunicacion_multimedia_field_fechahora_modificacion = date('Y-m-d H:i:s', time());
			}

			$this->registro_actual->comunicacion_multimedia_field_desc = $this->input->post('comunicacion_multimedia_field_desc');
			$this->registro_actual->comunicacion_producto_id = $this->input->post('comunicacion_producto_id');
			$this->registro_actual->comunicacion_multimedia_tipo_id = $this->input->post('comunicacion_multimedia_tipo_id');
	...
	private function _view()
	{
		...
		//------------ [select / checkbox / radio comunicacion_multimedia_tipo] :(
		$comunicacion_multimedia_tipo = new Comunicacion_Multimedia_Tipo();
		$q = $comunicacion_multimedia_tipo->get_all();
		$config = array();
		$config['fields'] = array('comunicacion_multimedia_tipo_field_desc');
		$config['select'] = true;
		$this->template['comunicacion_multimedia_tipo_id']=$this->_create_html_options($q, $config);
		//------------ [fin select / checkbox / radio comunicacion_multimedia_tipo]
		...
		$config['select'] = true;
		$this->template['comunicacion_producto_id']=$this->_create_html_options($q, $config);
		//------------ [fin select / checkbox / radio publicidad_producto_id]

		$this->template['comunicacion_multimedia_imagen'] = $this->registro_actual ? $this->registro_actual->Comunicacion_Multimedia_Imagen : array();
	...
	private function _validar_formulario() {
		...
		$this->form_validation->set_rules('comunicacion_multimedia_field_desc',$this->marvin->mysql_field_to_human('comunicacion_multimedia_field_desc'),
				'trim|max_length[255]|required' );

		$this->form_validation->set_rules('comunicacion_multimedia_tipo_id',$this->marvin->mysql_field_to_human('comunicacion_multimedia_tipo_id'),
				'trim|is_natural_no_zero' );

		$this->form_validation->set_rules('comunicacion_producto_id',$this->marvin->mysql_field_to_human('comunicacion_producto_id'),
				'trim|is_natural_no_zero' );
	...
	public function del_image( $id_registro = FALSE, $id_imagen= FALSE, $subfix = 'imagen'  )
	{
		$rs		= $this->backend->del_image($id_registro, $id_imagen, $subfix);
	}
