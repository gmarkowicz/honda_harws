<?php
/*
todo: id unico, escriba el campo ktype - DONE, tambien chequea la cantidad de digitos
*/

define('ID_SECCION',1064);
class Vin_Procedencia_Ktype_Abm extends Backend_Controller{
	...
	public function add()
	{
		...
		{		
			if ($this->_validar_formulario() == TRUE)
			{
				...
				$this->registro_actual = new Vin_Procedencia_Ktype();
	...
	private function _grabar_registro_actual()
	{
		try
		{
			...
			*/
			...
			$this->registro_actual->vin_procedencia_ktype_field_desc = $this->input->post('vin_procedencia_ktype_field_desc');
			$this->registro_actual->vin_procedencia_ktype_field_ktype = $this->input->post('vin_procedencia_ktype_field_ktype');
	...
	private function _validar_formulario() {
		...
		$this->form_validation->set_rules('id',$this->marvin->mysql_field_to_human('id'),
				'trim|exact_length[3]|required|my_unique_db[Vin_Procedencia_Ktype.id '.$id.']' );
		$this->form_validation->set_rules('vin_procedencia_ktype_field_desc',$this->marvin->mysql_field_to_human('vin_procedencia_ktype_field_desc'),
				'trim|required|max_length[255]' );
			$this->form_validation->set_rules('vin_procedencia_ktype_field_ktype',$this->marvin->mysql_field_to_human('vin_procedencia_ktype_field_ktype'),
				'trim|required|exact_length[2]' );
