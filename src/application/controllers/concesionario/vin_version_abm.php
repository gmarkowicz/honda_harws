...
/*		if($this->registro_actual)
			...
			'trim|max_length[1]|my_unique_db[Vin_Version.id '.$id.' Vin_Version.vin_version_id '.$vin_version_id.' Vin_Version.vin_modelo_id '.$vin_modelo_id.']' );
		else*/
		
		

		$this->form_validation->set_rules('vin_version_id',$this->marvin->mysql_field_to_human('vin_version_id'),
			'trim|max_length[1]|required|my_unique_db[Vin_Version.id '.$id.' Vin_Version.vin_version_id '.$this->input->post('vin_version_id').' Vin_Version.vin_modelo_id '.$this->input->post('vin_modelo_id').']' );


		$this->form_validation->set_rules('vin_modelo_id',$this->marvin->mysql_field_to_human('vin_modelo_id'),
				'trim|max_length[3]|required|my_db_value_exist[Vin_Modelo.id]' );
