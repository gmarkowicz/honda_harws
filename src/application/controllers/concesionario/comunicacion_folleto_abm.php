<?php
define('ID_SECCION',404);
class Comunicacion_Folleto_Abm extends Backend_Controller{
	...
	function __construct()
	{
		...
		$this->load->config('adjunto/' . strtolower($this->model) . '_pdf');
	...
	public function add()
	{
		if($this->input->post('_submit'))
		{
			if ($this->_validar_formulario() == TRUE)
			{
				...
				$this->registro_actual = new Comunicacion_Folleto();
	...
	private function _grabar_registro_actual()
	{
		try
		{
			...
			{
				$this->registro_actual->comunicacion_folleto_field_admin_alta_id = $this->session->userdata('admin_id');
				$this->registro_actual->comunicacion_folleto_field_fechahora_alta = date('Y-m-d H:i:s', time());
			}
			else if($this->router->method == 'edit' )
			{
				$this->registro_actual->comunicacion_folleto_field_admin_modifica_id = $this->session->userdata('admin_id');
				$this->registro_actual->comunicacion_folleto_field_fechahora_modificacion = date('Y-m-d H:i:s', time());
			}

			$this->registro_actual->comunicacion_folleto_field_desc = $this->input->post('comunicacion_folleto_field_desc');
	...
	private function _view()
	{
		...
		$this->template['comunicacion_folleto_imagen'] = $this->registro_actual ? $this->registro_actual->Comunicacion_Folleto_Imagen : array();
		$this->template['comunicacion_folleto_pdf'] = $this->registro_actual ? $this->registro_actual->Comunicacion_Folleto_Pdf : array();
	...
	private function _validar_formulario() {
		...
		$this->form_validation->set_rules('comunicacion_folleto_field_desc',$this->marvin->mysql_field_to_human('comunicacion_folleto_field_desc'),
