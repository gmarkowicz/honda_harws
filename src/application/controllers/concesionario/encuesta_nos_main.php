...
class Encuesta_Nos_Main extends Backend_Controller {
	...
	var $default_valid_fields = array(
		...
		'encuesta_nos_estado_field_desc'=>
			array(
				 'sql_filter'	=>FALSE,
				 'grid'			=>TRUE,
				 'width'		=>100,
		...
		'encuesta_nos_field_fechahora_alta'=>
			array(
				 'sql_filter'	=>FALSE,
				 'grid'			=>TRUE,
				 'width'		=>130,
...
*/
	...
	private function _validar_filtros() {		
		...
		$this->form_validation->set_rules('encuesta_nos_estado_id[]',$this->marvin->mysql_field_to_human('encuesta_nos_estado_id'),
			'trim'
		);
	...
	private function _view()
	{
		...
		//$this->output->enable_profiler();
		//------------ [select / checkbox / radio estado] :(
		$encuesta_nos_estado=new Encuesta_Nos_Estado();
		$q = $encuesta_nos_estado->get_all();
		$config=array();
		$config['fields'] = array('encuesta_nos_estado_field_desc');
		$config['select'] = FALSE;
		$this->template['encuesta_nos_estado_id']=$this->_create_html_options($q, $config);
		//------------ [fin select / checkbox / sucursal]
