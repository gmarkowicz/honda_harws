<?php
define('ID_SECCION',3075);
class Reclamo_Garantia_Estado_Abm extends Backend_Controller{
	...
	public function add()
	{
		...
		{		
			if ($this->_validar_formulario() == TRUE)
			{
				...
				$this->registro_actual = new Reclamo_Garantia_Estado();
	...
	private function _grabar_registro_actual()
	{
		try
		{
			...
			{
				$this->registro_actual->reclamo_garantia_estado_field_admin_alta_id = $this->session->userdata('admin_id');
				$this->registro_actual->reclamo_garantia_estado_field_fechahora_alta = date('Y-m-d H:i:s', time());
			}
			else if($this->router->method == 'edit' )
			{
				$this->registro_actual->reclamo_garantia_estado_field_admin_modifica_id = $this->session->userdata('admin_id');
				$this->registro_actual->reclamo_garantia_estado_field_fechahora_modificacion = date('Y-m-d H:i:s', time());
			}
			*/
			$this->registro_actual->id = $this->input->post('id');
			$this->registro_actual->reclamo_garantia_estado_field_desc = $this->input->post('reclamo_garantia_estado_field_desc');
	...
	private function _validar_formulario() {
		...
		$this->form_validation->set_rules('id',$this->marvin->mysql_field_to_human('id'),
				'trim|max_length[2]|numeric|required|my_unique_db[Reclamo_Garantia_Estado.id '.$id.']' );
		$this->form_validation->set_rules('reclamo_garantia_estado_field_desc',$this->marvin->mysql_field_to_human('reclamo_garantia_estado_field_desc'),
				'trim|max_length[255]|required||my_unique_db[Reclamo_Garantia_Estado.reclamo_garantia_estado_field_desc '.$id.']' );
