<?php
define('ID_SECCION',3034);

class Servicio_Circular_Main extends Backend_Controller {
	...
	var $default_valid_fields = array(
		...
		'_buscador_general'=>
			array(
					'sql_filter'	=>array('MATCH( 
												servicio_circular_field_titulo,
												servicio_circular_categoria_field_desc
		...
		'servicio_circular_field_titulo'=>
			array(
				 ...
				 'download'  	=>FALSE,
				 'print'  		=>TRUE,
				 'params'		=>'grid|sorteable|export|print|align-left|width-100|print|',
				 
			),

		'servicio_circular_field_fecha'=>
			array(
				 'sql_filter'	=>FALSE,
				 'grid'			=>TRUE,
				 'width'		=>70,
				 ...
				 'download'  	=>FALSE,
				 'print'	   	=>TRUE,
				 'params'		=>'grid|sorteable|export|print|align-left|width-100|',
				 
			),
		'servicio_circular_field_fecha_inicial'=>
			array(
				 'sql_filter'	=>array('DATE(servicio_circular_field_fecha) >= ?'),
				 'grid'			=>FALSE,
				 'width'		=>FALSE,
				 'sorteable'	=>FALSE,
				 'align'		=>FALSE,
		...
		'servicio_circular_field_fecha_final'=>
			array(
				 'sql_filter'	=>array('DATE(servicio_circular_field_fecha) >= ?'),
				 'grid'			=>FALSE,
				 'width'		=>FALSE,
		...
		'servicio_circular_categoria_id'=>
			array(
				 ...
				 'download'  	=>FALSE,
				 'print'  		=>TRUE,
				 'params'		=>'align-left|width-100|print|',
	...
	function Servicio_Circular_Main()
	{
	...
	private function _validar_filtros() {		
		...
		$this->form_validation->set_rules('servicio_circular_field_titulo',$this->marvin->mysql_field_to_human('servicio_circular_field_titulo'),
			'trim'
		);
		$this->form_validation->set_rules('servicio_circular_field_fecha_inicial',$this->marvin->mysql_field_to_human('servicio_circular_field_fecha_inicial'),
			'trim|my_form_date_reverse'
		);
		$this->form_validation->set_rules('servicio_circular_field_fecha_final',$this->marvin->mysql_field_to_human('servicio_circular_field_fecha_final'),
			'trim|my_form_date_reverse'
		);
		$this->form_validation->set_rules('servicio_circular_field_fecha',$this->marvin->mysql_field_to_human('servicio_circular_field_fecha'),
			'trim|my_form_date_reverse'
		);
		$this->form_validation->set_rules('servicio_circular_categoria_id[]',$this->marvin->mysql_field_to_human('servicio_circular_categoria_id'),
			'trim'
		);
	...
	private function _view()
	{
		...
		//------------ [select / checkbox / radio servicio_circular_categoria_id] :(
		$servicio_circular_categoria=new Servicio_Circular_Categoria();
		$q = $servicio_circular_categoria->get_all();
		//$q->WhereIn(' sucursal_id ', $this->session->userdata('sucursales'));
		$config=array();
		$config['fields'] = array('servicio_circular_categoria_field_desc');
		$config['select'] = FALSE;
		$this->template['servicio_circular_categoria_id']=$this->_create_html_options($q, $config);
		//------------ [fin select / checkbox / radio servicio_circular_categoria_id]
