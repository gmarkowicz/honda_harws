<?php
define('ID_SECCION',1025);
class Unidad_Estado_Garantia_abm extends Backend_Controller{
	...
	public function add()
	{
		...
		{		
			if ($this->_validar_formulario() == TRUE)
			{
				...
				$this->registro_actual = new Unidad_Estado_Garantia();
	...
	private function _grabar_registro_actual()
	{
		try
		{
			...
			*/
			$this->registro_actual->unidad_estado_garantia_field_desc = $this->input->post('unidad_estado_garantia_field_desc');
	...
	private function _validar_formulario() {
		...
		$this->form_validation->set_rules('unidad_estado_garantia_field_desc',$this->marvin->mysql_field_to_human('unidad_estado_garantia_field_desc'),
				'trim|max_length[255]|required|my_unique_db[Unidad_Estado_Garantia.unidad_estado_garantia_field_desc '.$id.']	' );
