<?php
define('ID_SECCION',1073);
class Empresa_abm extends Backend_Controller{
	...
	public function add()
	{
		...
		{		
			if ($this->_validar_formulario() == TRUE)
			{
				...
				$this->registro_actual = new Empresa();
	...
	private function _grabar_registro_actual()
	{
		try
		{
			...
			*/
			$this->registro_actual->empresa_field_desc = $this->input->post('empresa_field_desc');
			$this->registro_actual->empresa_field_id_sap = $this->input->post('empresa_field_id_sap');
	...
	private function _validar_formulario() {
		...
		$this->form_validation->set_message('is_natural_no_zero', $this->lang->line('form_seleccione'));
			...
			$this->form_validation->set_rules('empresa_field_id_sap',$this->marvin->mysql_field_to_human('empresa_field_id_sap'),
				'trim|max_length[7]|integer|my_unique_db[Empresa.empresa_field_id_sap '.$id.']' );
		$this->form_validation->set_rules('empresa_field_desc',$this->marvin->mysql_field_to_human('empresa_field_desc'),
