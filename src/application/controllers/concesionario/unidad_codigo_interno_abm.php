<?php
define('ID_SECCION',1022);
class Unidad_Codigo_Interno_abm extends Backend_Controller{
	...
	public function add()
	{
		...
		{		
			if ($this->_validar_formulario() == TRUE)
			{
				...
				$this->registro_actual = new Unidad_Codigo_Interno();
	...
	public function del()
	{
		...
		try {
			...
			$this->registro_actual->unlink('Many_Unidad');
	...
	private function _grabar_registro_actual()
	{
		try
		{
			...
			*/
			$this->registro_actual->unidad_codigo_interno_field_desc = $this->input->post('unidad_codigo_interno_field_desc');
			$this->registro_actual->unidad_codigo_interno_field_comentarios = $this->input->post('unidad_codigo_interno_field_comentarios');
	...
	private function _validar_formulario() {
		...
		$this->form_validation->set_rules('unidad_codigo_interno_field_desc',$this->marvin->mysql_field_to_human('unidad_codigo_interno_field_desc'),
				'trim|max_length[255]|required|my_unique_db[Unidad_Codigo_Interno.unidad_codigo_interno_field_desc '.$id.']' );

		$this->form_validation->set_rules('unidad_codigo_interno_field_comentarios',$this->marvin->mysql_field_to_human('unidad_codigo_interno_field_comentarios'),
