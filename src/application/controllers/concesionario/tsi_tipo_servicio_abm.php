<?php
define('ID_SECCION',3024);
class Tsi_Tipo_Servicio_Abm extends Backend_Controller{
	...
	public function add()
	{
		...
		{		
			if ($this->_validar_formulario() == TRUE)
			{
				...
				$this->registro_actual = new Tsi_Tipo_Servicio();
	...
	public function del()
	{
		...
		try {
			...
			//$this->registro_actual->unlink('Many_Tsi');
	...
	private function _grabar_registro_actual()
	{
		try
		{
			...
			*/
			$this->registro_actual->tsi_tipo_servicio_field_desc = $this->input->post('tsi_tipo_servicio_field_desc');

			$this->registro_actual->save();

			$this->registro_actual->unlink('Many_Tsi');
			$this->registro_actual->save();
			if($this->registro_actual!=FALSE && is_array($this->input->post('many_tsi')))
			{
					foreach($this->input->post('many_tsi') as $tsi_id) {
						$relacion=new Many_Tsi();
						$relacion->tsi_id = (int)$tsi_id;
						$relacion->tsi_tipo_servicio_id = $this->registro_actual->id;
						$relacion->save();
					}
			}
	...
	private function _validar_formulario() {
		...
		$this->form_validation->set_rules('tsi_tipo_servicio_field_desc',$this->marvin->mysql_field_to_human('tsi_tipo_servicio_field_desc'),
				'trim|max_length[255]|required|my_unique_db[Tsi_Tipo_Servicio.tsi_tipo_servicio_field_desc '.$id.']' );


		$this->form_validation->set_rules('many_tsi[]',$this->marvin->mysql_field_to_human('tsi_id'),
			'trim' );
