<?php
define('ID_SECCION',1032);
class Provincia_abm extends Backend_Controller{
	...
	public function add()
	{
		...
		{		
			if ($this->_validar_formulario() == TRUE)
			{
				...
				$this->registro_actual = new Provincia();
	...
	private function _grabar_registro_actual()
	{
		try
		{
			...
			*/
			$this->registro_actual->provincia_field_desc = $this->input->post('provincia_field_desc');
	...
	private function _validar_formulario() {
		...
		$this->form_validation->set_rules('provincia_field_desc',$this->marvin->mysql_field_to_human('provincia_field_desc'),
				'trim|max_length[255]|required|callback_valid_provincia' );

		$this->form_validation->set_rules('pais_id',$this->marvin->mysql_field_to_human('pais_id'),
				'trim|max_length[2]|my_db_value_exist[Pais.id]' );
	...
	function valid_provincia($provincia)
	{
		$this->form_validation->set_message('valid_provincia', $this->lang->line('form_campo_duplicado'));
		...
		->from('Provincia')
		->where('provincia_field_desc = ?',$this->input->post('provincia_field_desc'))
		->addWhere('pais_id = ?',$this->input->post('pais_id'))
		->addWhere('id != ?',$id);
