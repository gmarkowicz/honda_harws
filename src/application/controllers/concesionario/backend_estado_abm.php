...
class Backend_Estado_abm extends Backend_Controller{
	...
	public function add()
	{
		...
		{		
			if ($this->_validar_formulario() == TRUE)
			{
				...
				$this->registro_actual = new Backend_Estado();
	...
	private function _grabar_registro_actual()
	{
		try
		{
			...
			*/
			$this->registro_actual->backend_estado_field_desc = $this->input->post('backend_estado_field_desc');
	...
	private function _validar_formulario() {
		...
		$this->form_validation->set_rules('backend_estado_field_desc',$this->marvin->mysql_field_to_human('backend_estado_field_desc'),
				'trim|max_length[255]' );
