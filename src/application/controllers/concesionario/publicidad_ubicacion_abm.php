<?php
define('ID_SECCION',4013);
class Publicidad_Ubicacion_Abm extends Backend_Controller{
	...
	public function add()
	{
		...
		{		
			if ($this->_validar_formulario() == TRUE)
			{
				...
				$this->registro_actual = new Publicidad_Ubicacion();
	...
	private function _grabar_registro_actual()
	{
		try
		{
			...
			{
				$this->registro_actual->publicidad_ubicacion_field_admin_alta_id = $this->session->userdata('admin_id');
				$this->registro_actual->publicidad_ubicacion_field_fechahora_alta = date('Y-m-d H:i:s', time());
			}
			else if($this->router->method == 'edit' )
			{
				$this->registro_actual->publicidad_ubicacion_field_admin_modifica_id = $this->session->userdata('admin_id');
				$this->registro_actual->publicidad_ubicacion_field_fechahora_modificacion = date('Y-m-d H:i:s', time());
			}
			*/
			$this->registro_actual->publicidad_ubicacion_field_desc = $this->input->post('publicidad_ubicacion_field_desc');
	...
	private function _validar_formulario() {
		...
		$this->form_validation->set_rules('publicidad_ubicacion_field_desc',$this->marvin->mysql_field_to_human('publicidad_ubicacion_field_desc'),
