...
*/
define('ID_SECCION',1063);
class Vin_Modelo_abm extends Backend_Controller{
	...
	public function add()
	{
		...
		{		
			if ($this->_validar_formulario() == TRUE)
			{
				...
				$this->registro_actual = new Vin_Modelo();
	...
	private function _validar_formulario() {
		...
		$this->form_validation->set_rules('id',$this->marvin->mysql_field_to_human('id'),
				'trim|exact_length[3]|required|my_unique_db[Vin_Modelo.id '.$id.']' );
