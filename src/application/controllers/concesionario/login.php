...
class Login extends CI_Controller{
	...
	public function act_as($id = FALSE)
	{
		if($this->session->userdata('admin_field_super_admin')===TRUE)
		{
			$this->root_admin = FALSE;
			if($id!= $this->session->userdata('admin_id'))
				$this->root_admin = $this->session->userdata('admin_id');
			$this->act_as = $id;
			$this->_login();
		}
		else
	...
	public function logoff()
	{
		...
		if($this->session->userdata('root_admin'))
		{
			$this->root_admin = FALSE;
			$this->act_as = $this->session->userdata('root_admin');
			$this->_login();
		}
	...
	private function _login()
	{
		...
		if($this->root_admin)
		{
			$this->session->set_userdata('root_admin', $this->root_admin);
		}
		$admin=new Admin();
		$q=$admin->get_all();
		if(!$this->act_as)
		{
			...
			//$q->whereIn('id',array(1,3));
		}
		else
		{
			$q->addWhere('id = ?' ,$this->act_as);
		}
		$admin = $q->fetchOne();
