<?php
define('ID_SECCION',2012);
class Noticia_Seccion_abm extends Backend_Controller{
	...
	public function add()
	{
		...
		{		
			if ($this->_validar_formulario() == TRUE)
			{
				...
				$this->registro_actual = new Noticia_Seccion();
	...
	private function _grabar_registro_actual()
	{
		try
		{
			...
			*/
			$this->registro_actual->noticia_seccion_field_desc = $this->input->post('noticia_seccion_field_desc');
	...
	private function _validar_formulario() {
		...
		$this->form_validation->set_rules('noticia_seccion_field_desc',$this->marvin->mysql_field_to_human('noticia_seccion_field_desc'),
