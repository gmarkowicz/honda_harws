<?php
define('ID_SECCION',1042);
class Auto_Modelo_abm extends Backend_Controller{
	...
	public function add()
	{
		...
		{		
			if ($this->_validar_formulario() == TRUE)
			{
				...
				$this->registro_actual = new Auto_Modelo();
	...
	private function _grabar_registro_actual()
	{
		try
		{
			...
			*/
			$this->registro_actual->auto_modelo_field_desc = $this->input->post('auto_modelo_field_desc');
			$this->registro_actual->auto_marca_id = $this->input->post('auto_marca_id');
	...
	private function _view()
	{
		...
		//------------ [select / checkbox / radio auto_marca_id] :(
		...
		//------------ [fin select / checkbox / radio auto_marca_id]
	...
	private function _validar_formulario() {
		...
		$this->form_validation->set_rules('auto_modelo_field_desc',$this->marvin->mysql_field_to_human('auto_modelo_field_desc'),
				'trim|max_length[255]|required|callback_valid_modelo' );
	...
	function valid_modelo($modelo)
	{
		
		$this->form_validation->set_message('valid_modelo', $this->lang->line('form_campo_duplicado'));
		...
		->from('Auto_Modelo')
		->where('auto_modelo_field_desc = ?',$modelo)
		->addWhere('auto_marca_id = ?',$this->input->post('auto_marca_id'))
		->addWhere('id != ?',$id);
