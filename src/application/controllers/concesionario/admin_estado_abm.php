<?php
define('ID_SECCION',1017);
class Admin_Estado_abm extends Backend_Controller{
	...
	public function add()
	{
		...
		{		
			if ($this->_validar_formulario() == TRUE)
			{
				...
				$this->registro_actual = new Admin_Estado();
	...
	private function _grabar_registro_actual()
	{
		try
		{
			...
			*/
			$this->registro_actual->admin_estado_field_desc = $this->input->post('admin_estado_field_desc');
	...
	private function _validar_formulario() {
		...
		$this->form_validation->set_rules('admin_estado_field_desc',$this->marvin->mysql_field_to_human('admin_estado_field_desc'),
