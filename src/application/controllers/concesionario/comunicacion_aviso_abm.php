<?php
define('ID_SECCION',4042);
class Comunicacion_Aviso_Abm extends Backend_Controller{
	...
	function __construct()
	{
		...
		$this->load->config('adjunto/' . strtolower($this->model) . '_adjunto');
	...
	public function add()
	{
		if($this->input->post('_submit'))
		{
			if ($this->_validar_formulario() == TRUE)
			{
				...
				$this->registro_actual = new Comunicacion_Aviso();
	...
	private function _grabar_registro_actual()
	{
		try
		{
			...
			{
				$this->registro_actual->comunicacion_aviso_field_admin_alta_id = $this->session->userdata('admin_id');
				$this->registro_actual->comunicacion_aviso_field_fechahora_alta = date('Y-m-d H:i:s', time());
			}
			else if($this->router->method == 'edit' )
			{
				$this->registro_actual->comunicacion_aviso_field_admin_modifica_id = $this->session->userdata('admin_id');
				$this->registro_actual->comunicacion_aviso_field_fechahora_modificacion = date('Y-m-d H:i:s', time());
			}

			$this->registro_actual->comunicacion_aviso_field_fecha_publicacion = date('Y-m-d H:i:s', strtotime($this->input->post('comunicacion_aviso_field_fecha_publicacion')));
			$this->registro_actual->comunicacion_aviso_field_desc = $this->input->post('comunicacion_aviso_field_desc');
	...
	private function _view()
	{
		...
		$this->template['comunicacion_aviso_imagen'] = $this->registro_actual ? $this->registro_actual->Comunicacion_Aviso_Imagen : array();

		$this->template['comunicacion_aviso_adjunto'] = $this->registro_actual ? $this->registro_actual->Comunicacion_Aviso_Adjunto : array();
	...
	private function _validar_formulario() {
		...
		$this->form_validation->set_rules('comunicacion_aviso_field_desc',$this->marvin->mysql_field_to_human('comunicacion_aviso_field_desc'),
		...
		$this->form_validation->set_rules('comunicacion_aviso_field_fecha_publicacion',$this->marvin->mysql_field_to_human('comunicacion_aviso_field_fecha_publicacion'),
