<?php
define('ID_SECCION',3029);
class Tsi_Promocion_Abm extends Backend_Controller{
	...
	public function add()
	{
		...
		{		
			if ($this->_validar_formulario() == TRUE)
			{
				...
				$this->registro_actual = new Tsi_Promocion();
	...
	private function _grabar_registro_actual()
	{
		try
		{
			...
			{
				$this->registro_actual->tsi_promocion_field_admin_alta_id = $this->session->userdata('admin_id');
				$this->registro_actual->tsi_promocion_field_fechahora_alta = date('Y-m-d H:i:s', time());
			}
			else if($this->router->method == 'edit' )
			{
				$this->registro_actual->tsi_promocion_field_admin_modifica_id = $this->session->userdata('admin_id');
				$this->registro_actual->tsi_promocion_field_fechahora_modificacion = date('Y-m-d H:i:s', time());
			}
			
			$this->registro_actual->tsi_promocion_field_desc = $this->input->post('tsi_promocion_field_desc');
			$this->registro_actual->tsi_promocion_field_fecha_inicio 	= $this->input->post('tsi_promocion_field_fecha_inicio');
			$this->registro_actual->tsi_promocion_field_fecha_fin 		= $this->input->post('tsi_promocion_field_fecha_fin');
	...
	private function _validar_formulario() {
		...
		$this->form_validation->set_rules('tsi_promocion_field_desc',$this->marvin->mysql_field_to_human('tsi_promocion_field_desc'),
				'trim|max_length[255]|required|my_unique_db[Tsi_Promocion.tsi_promocion_field_desc '.$id.']' );
		
		$this->form_validation->set_rules('tsi_promocion_field_fecha_inicio',$this->marvin->mysql_field_to_human('tsi_promocion_field_fecha_inicio'),
				'trim|required|my_form_date_reverse|my_valid_date[y-m-d,-,0]' );
		
		$this->form_validation->set_rules('tsi_promocion_field_fecha_fin',$this->marvin->mysql_field_to_human('tsi_promocion_field_fecha_fin'),
				'trim|required|my_form_date_reverse|my_valid_date[y-m-d,-,0]|callback_valid_fecha_fin' );
	...
	function valid_fecha_fin($fecha)
	{
		
		$this->form_validation->set_message('valid_fecha_fin', sprintf($this->lang->line('valid_fecha_fin'), $this->lang->line('fecha_fin'), $this->lang->line('fecha_inicio')));
		
		
		$inicio = date_create($this->form_validation->my_form_date_reverse($this->input->post('tsi_promocion_field_fecha_inicio')));
		$fin = date_create($this->form_validation->my_form_date_reverse($this->input->post('tsi_promocion_field_fecha_fin')));
