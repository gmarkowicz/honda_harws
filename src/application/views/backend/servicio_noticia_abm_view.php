...
<form autocomplete="off" enctype="multipart/form-data" method="post" action="<?php echo current_url();?>">
		...
		<ul>	
				...
				<fieldset>
				<legend><?=$this->marvin->mysql_field_to_human('servicio_noticia_id');?></legend>
				<li class="unitx4 f-left">
				

					<?php
						$config	=	array(
							'field_name'		=>'servicio_noticia_field_titulo',
							'field_req'		=>TRUE,
		...
		<li class="unitx4 f-left">
			<?php
					$config	=	array(
							'field_name'		=>'servicio_noticia_field_fecha',
							'field_req'		=>FALSE,
		...
		*/
		...
		<?php
			...
			$config['model'] = strtolower($this->model);
			if(isset($servicio_noticia_adjunto) && count($servicio_noticia_adjunto)>0)
			{
				$config['adjuntos'] = $servicio_noticia_adjunto;
			}else{
