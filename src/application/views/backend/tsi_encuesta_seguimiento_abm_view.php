...
<script type="text/javascript">
$(document).ready(function(){
	$('input._unidad_field').attr("readonly", true); 
...
$(document).ready(function(){
	...
	$('select[name="tsi_id"], #livetsi').livequery('change', function(e)
	{
		
		var input = $('select[name="tsi_id"]');
		$.manageAjax.add('cola_cliente_tsi', 
		{
				
				
				beforeSend: function()
				{
					$('._ajax_append').append('<div class="_ajax_loading _cola_cliente_tsi"></div>');
					$("#cliente_tsi").html('');
				},
				cache: false,
				type: "POST",
				url: "<?=$this->config->item('base_url');?><?=$this->config->item('backend_root');?>ajax/get_cliente_tsi",
				data: "tsi_id="+input.val(),
				success: function(respuesta)
				{
					$('._cola_cliente_tsi').remove();
					$("#cliente_tsi").html(respuesta);
...
<?php if(in_array(set_value('tsi_encuesta_seguimiento_estado_id'),$this->bloqueados) && !$this->backend->_permiso('admin')):?>
<script type="text/javascript">
					$(document).ready(function(){
						block_input();
					}); 
