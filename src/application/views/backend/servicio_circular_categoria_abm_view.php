...
<form autocomplete="off" enctype="multipart/form-data" method="post" action="<?php echo current_url();?>">
		...
		<ul>	
				...
				<fieldset>
				<legend><?=$this->marvin->mysql_field_to_human('servicio_circular_categoria_id');?></legend>
				<li class="unitx4 f-left">
				

					<?php
						$config	=	array(
							'field_name'		=>'servicio_circular_categoria_field_desc',
							'field_req'		=>TRUE,
		...
		*/
		...
		<?php
			...
			$config['model'] = strtolower($this->model);
			if(isset(MODELO_ADJUNTO) && count($MODELO_ADJUNTO)>0)
			{
				$config['adjuntos'] = $MODELO_ADJUNTO;
			}
